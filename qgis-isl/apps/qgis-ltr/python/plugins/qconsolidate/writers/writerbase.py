...
class WriterTaskBase(QgsTask):
    ...
    def run(self):
        ...
        for count, layer in enumerate(layers.values()):
            QgsMessageLog.logMessage(QCoreApplication.translate('WriterTaskBase', 'Consolidating {layer}.').format(layer=layer.name()), 'QConsolidate', Qgis.Info)
            if self.isCanceled():
                break
    ...
    def exportVectorLayer(self, layer, destinationFile, asLayer=False):
        ...
        if error != QgsVectorFileWriter.NoError:
            QgsMessageLog.logMessage(QCoreApplication.translate('WriterTaskBase', 'Failed to export layer "{layer}": {message}.').format(layer=layer.name(), message=msg), 'QConsolidate', Qgis.Warning)
            success = False
    ...
    def exportRasterLayer(self, layer, destinationFile, options=None):
        ...
        if not pipe.set(provider.clone()):
            QgsMessageLog.logMessage(QCoreApplication.translate('WriterTaskBase', 'Failed to export layer "{layer}": Cannot set pipe provider.').format(layer=layer.name()), 'QConsolidate', Qgis.Warning)
            return False, None
        ...
        if not pipe.insert(1, nuller):
            QgsMessageLog.logMessage(QCoreApplication.translate('WriterTaskBase', 'Failed to export layer "{layer}": Cannot set pipe nuller.').format(layer=layer.name()), 'QConsolidate', Qgis.Warning)
            return False, None
        ...
        if error != QgsRasterFileWriter.NoError:
            QgsMessageLog.logMessage(QCoreApplication.translate('WriterTaskBase', 'Failed to export layer "{layer}": {message}.').format(layer=layer.name(), message=error), 'QConsolidate', Qgis.Warning)
            success = False
