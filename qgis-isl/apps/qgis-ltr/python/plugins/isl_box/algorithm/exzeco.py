...
# [x] Ne pas définir de raster au début de l'itération, utiliser les OUTPUT temporaires directement
#     -> garder pour suppression aisée des fichiers intermédiaires au besoin ?
# [ ] Check issue github
...
class Exzeco(QgsProcessingAlgorithm):
    """
    Ce script est l'adaptation de la méthode ExZEco développée par le Cerema
    pour définir le ruissellement sur l'arc méditerranéen.
    ...
    def shortHelpString(self):
        return """
            ...
            <p><img src="{image_path}" alt="" align="left" width="80" height="80" style="margin: 2px;"/>Exzeco est une m&eacute;thode simple, qui permet, &agrave; partir de la topographie, d&rsquo;obtenir des emprises potentiellement inondables sur de petits bassins versants.</p>
        ...
        """.format(image_path=os.path.join(os.path.dirname(__file__), "..", "icons", "exzeco.png"))

    def helpUrl(self):
        return "https://www.cerema.fr/system/files/documents/2020/07/methode_exzeco_25mai2020.pdf"

    def icon(self):
        return QIcon(os.path.join(os.path.dirname(__file__), "..", "icons", "exzeco.png"))
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        feedback = QgsProcessingMultiStepFeedback(nbFeedbackStep * iteration, model_feedback)
        outputs = {}
        ...
        for i in range(iteration):
            ...
            aleatoire = QgsProcessingUtils.generateTempFilename(str(i).zfill(lenIter) + "_aleatoire.tif")
            MNTBruite = QgsProcessingUtils.generateTempFilename(str(i).zfill(lenIter) + "_MNTBruite.tif")
            surfacesDrainees = QgsProcessingUtils.generateTempFilename(str(i).zfill(lenIter) + "_surfacesDrainees.tif")
            surfacesDraineesSeuil = QgsProcessingUtils.generateTempFilename(
                str(i).zfill(lenIter) + "_surfacesDraineesSeuil.tif"
            )
            surfacesDraineesSeuilMax = QgsProcessingUtils.generateTempFilename(
                str(i).zfill(lenIter) + "_surfacesDraineesSeuilMax.tif"
            )

            feedback.setProgressText("Itération {}/{} : Création du raster aléatoire".format(i + 1, iteration))
            feedback.setCurrentStep(nbFeedbackStep * i)
            ...
            feedback.setCurrentStep(nbFeedbackStep * i + 1)
            if feedback.isCanceled():
                return {}
            ...
            feedback.setCurrentStep(nbFeedbackStep * i + 2)
            if feedback.isCanceled():
                return {}
            ...
            feedback.setCurrentStep(nbFeedbackStep * i + 3)
            if feedback.isCanceled():
                return {}
            ...
            feedback.setCurrentStep(nbFeedbackStep * i + 4)
            if feedback.isCanceled():
                return {}
            ...
            if supprFichiersIntermed:
                feedback.setProgressText(
                    "Itération {}/{} : Suppression des fichiers intermédaires".format(i + 1, iteration)
                )
                feedback.setCurrentStep(nbFeedbackStep * i + 5)
                ...
                # Supprime le dossier "grassdata" contenant des fichiers temporaires créés par les alg GRASS
                grassDir = os.path.join(Path(aleatoire).parent.parent, "grassdata")
                shutil.rmtree(grassDir)
